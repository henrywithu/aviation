...
export class Terrain{
 readonly group=new Group();readonly material:ShaderMaterial;private offset=0;private lock=0;private wasThermal=false;
 constructor(model:GLTF,noise:Texture,city:Texture){const source=model.scene.getObjectByName('Land-A') as Mesh;const base=(source.material as unknown as {map:Texture}).map;
 ...
 for(let z=0;z<6;z++)for(let x=-1;x<=2;x++){const m=new Mesh(source.geometry,this.material);m.scale.setScalar(.04);m.position.set(x*15.99,0,z*15.99);m.userData.z=z*15.99;m.frustumCulled=false;this.group.add(m)}
 }
